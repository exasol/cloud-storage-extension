...
class ImportMetadataSuite extends BaseSuite {
  ...
  test("`run` should create a list of files names") {
    ...
    verify(exaIter, times(1))
      .emit(s"$resourcePath/import/parquet/sales_positions1.snappy.parquet", "0")
    verify(exaIter, times(1))
      .emit(s"$resourcePath/import/parquet/sales_positions2.snappy.parquet", "1")
    verify(exaIter, times(1))
      .emit(s"$resourcePath/import/parquet/sales_positions_small.snappy.parquet", "0")
  }
