...
```sh
sha256sum exasol-cloud-storage-extension-2.8.6.jar
```
...
`target/exasol-cloud-storage-extension-2.8.6.jar`.
...
```sh
curl -X PUT -T exasol-cloud-storage-extension-2.8.6.jar \
...
CREATE OR REPLACE JAVA SET SCRIPT IMPORT_PATH(...) EMITS (...) AS
  ...
  %jar /buckets/bfsdefault/<BUCKET>/exasol-cloud-storage-extension-2.8.6.jar;
/
